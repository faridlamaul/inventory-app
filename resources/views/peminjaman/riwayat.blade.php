                                    ...
                                    <div>
                                        <div class="uk-card uk-card-default">
                                            ...
                                            <div class="uk-card-body">
                                                ...
                                                <p class="cut-text">{{ $transaksi->description }}</p>
                                                <h5>Status :
                                                    @if ($transaksi->end_date >= \Carbon\Carbon::now() && $transaksi->start_date <= \Carbon\Carbon::now())
                                                        <span class="uk-text-success">Active</span>
                                                ...
                                                <h6>
                                                    @if ($transaksi->end_date >= \Carbon\Carbon::now() && $transaksi->start_date <= \Carbon\Carbon::now())
                                                        <span class="uk-text-muted">Masih dalam masa penyewaan</span>
                                                    @elseif ($transaksi->end_date >= \Carbon\Carbon::now() && $transaksi->start_date >= \Carbon\Carbon::now())
                                        ...
                                        <div class="uk-margin">
                                            @if ($transaksi->end_date == null)
                                                <button class="uk-button uk-button-primary" type="button"
                                                    uk-toggle="target: #modal-{{ $transaksi->id }}">Ajukan
                                                    Pengembalian</button>
                                            @endif
                                            <!-- This is the modal -->
                                            <div id="modal-{{ $transaksi->id }}" uk-modal>
                                                <div class="uk-modal-dialog uk-modal-body">
                                                    ...
                                                    <form action="{{ url('/riwayat/kembalikan/' . $transaksi->id) }}"
                                                        method="POST">
                                                        @csrf
